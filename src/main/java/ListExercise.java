...
public class ListExercise {
    ...
    public List<Integer> createList(){
        //LinkedList is not imported so ArrayList it is
    ...
    public int get(List<Integer> list, int index){
        // The if statement is to limit attempts to access indexes which do not exist in the ArrayList.
        // It is not checked here but is something that I figure would be handled in real applications 
        if (index < list.size() && index >= 0){
            return list.get(index);
        } else {
            System.out.println("That index is not assigned in this List.");
        }
        return 0;
    }
    ...
    public void removeFromList(List<Integer> list, int position){
        // the if statement is to limit attempts to modify indexes which do not exist in the ArrayList
